...
describe("POST /login", function () {
  it("Verify Success Login with valid email and password", async function () {
    const response = await LoginPage.login(
      "inovasi122@gmail.com",
      "tandacinta"
    );

    // Menggunakan helper untuk memeriksa response
    checkSuccessResponse(response, "SUCCESS_LOGIN", "Anda Berhasil Login");
  });

  it("Verify Success Login with additional parameter", async function () {
    const response = await LoginPage.loginWithAdditionalParams(
      "inovasi122@gmail.com",
      "tandacinta",
      "uwow nih"
    );
  ...
  it("Verify Success Login with Random Authorization", async function () {
    const response = await LoginPage.loginWithAuthorization(
      "inovasi122@gmail.com",
      "tandacinta",
      "asjdhgagduahuidghaiduawgdiuwaghsgjhagdjhgdshjsgd"
    );
    checkSuccessResponse(response, "SUCCESS_LOGIN", "Anda Berhasil Login");
  });

  it("Verify Failed Login with invalid email and valid password", async function () {
    const response = await LoginPage.loginWithInvalidEmail(
      "barru.kurniawan",
      "sman60jakarta"
    );
    checkErrorResponse(response, 420, "Cek kembali email anda"); // Sesuaikan dengan pesan error yang diharapkan
  });

  it("Verify Failed Login with random email and valid password", async function () {
    try {
      const response = await LoginPage.loginWithRandomEmail(
        "barru.asdajsdasdasd@bkabka.com",
        "sman60jakarta"
      );
      // Memastikan status code dan pesan error yang sesuai
      checkErrorResponse(response, 420, "Cek kembali email anda"); // Sesuaikan dengan status code dan pesan error yang diharapkan
    } catch (error) {
      // Jika ada kesalahan atau assertion gagal, tangkap dan lempar sebagai error baru
...
// describe("POST /login", function () {
//   // JUDUL, JADI SINGKAT AJA

//   it("Verify Failed Login with random email and valid password", async function () {
//     const response = await request_url.post("/login").send({
//       email: "barru.asdajsdasdasd@bkabka.com",
...
//       .post("/login")
//       .send({ email: "088823772363", password: "sman60jakarta" });
...
//     const response = await request_url
//       .post("/login")
//       .send({ email: "barru.kurniawan@gmail.com", password: "" });
...
//       password:
//         "SELECT count(*) FROM Users WHERE Username='jebol' or 1=1--' AND Password='email'",
//     });

//     const isi_data = response.body;

//     assert(response.body.status).to.eql("FAILED_LOGIN");
//     assert(response.body.message).to.eql(
...
//         const response = await request_url.post("/login").send({
//           email: "dicobain@tester.com",
//           password: "sampeblokir",
//           ip_address: "127.107.42.1",
//         });
...
//     assert(response.body.status).to.eql("FAILED_LOGIN");
//     assert(response.body.data).to.eql("Cek Formulir Anda");
//     assert(isi_data).to.include.keys("data", "message", "status");
//   });

//   it("Verify Failed Login with Max Char in Password Field", async function () {
//     let max_password = Array.from(Array(55), () =>
//       Math.floor(Math.random() * 36).toString(36)
//     ).join("");
//     const response = await request_url
//       .post("/login")
